...
inline char *SZ_compress_Interp(SZ::Config &conf, T *data, size_t &outSize) {
...
template<class T, SZ::uint N>
inline char* SZ_compress_Interp(SZ::Config& conf, T* data, size_t& outSize, std::vector<unsigned char>& compr) {
    ...
    auto sz = SZ::SZInterpolationCompressor<T, N, SZ::LinearQuantizer<T>, SZ::HuffmanEncoder<int>, SZ::Lossless_zstd>(
        ...
        SZ::HuffmanEncoder<int>(),
        SZ::Lossless_zstd());
    char* cmpData = (char*)sz.compress(conf, data, outSize, compr);
    return cmpData;
}

template<class T, SZ::uint N>
inline char* SZ_compress_Interp(SZ::Config& conf, T* data, size_t& outSize, std::vector<unsigned char>& compr, size_t& offset) {
    ...
    auto sz = SZ::SZInterpolationCompressor<T, N, SZ::LinearQuantizer<T>, SZ::HuffmanEncoder<int>, SZ::Lossless_zstd>(
        ...
        SZ::HuffmanEncoder<int>(),
        SZ::Lossless_zstd());
    char* cmpData = (char*)sz.compress(conf, data, outSize, compr, offset);
    return cmpData;
}

template<class T, SZ::uint N>
inline void SZ_decompress_Interp(const SZ::Config &conf, char *cmpData, size_t cmpSize, T *decData) {
    assert(conf.cmprAlgo == SZ::ALGO_INTERP);
...
template<class T, SZ::uint N>
inline double do_not_use_this_interp_compress_block_test(T *data, std::vector<size_t> dims, size_t num,
...
template<class T, SZ::uint N>
inline char *SZ_compress_Interp_lorenzo(SZ::Config &conf, T *data, size_t &outSize) {
...
template<class T, SZ::uint N>
inline char* SZ_compress_Interp_lorenzo(SZ::Config& conf, T* data, size_t& outSize, std::vector<unsigned char>& compr) {
    assert(conf.cmprAlgo == SZ::ALGO_INTERP_LORENZO);
    ...
    if (sampling_num == conf.num) {
        conf.cmprAlgo = SZ::ALGO_INTERP;
        return SZ_compress_Interp<T, N>(conf, data, outSize, compr);
    }
    ...
    if (useInterp) {
        ...
        conf = lorenzo_config;
        double tuning_time = timer.stop();
        return SZ_compress_LorenzoReg<T, N>(conf, data, outSize, compr);
    }
...
template<class T, SZ::uint N>
inline char* SZ_compress_Interp_lorenzo(SZ::Config& conf, T* data, size_t& outSize, std::vector<unsigned char>& compr, size_t& offset) {
    assert(conf.cmprAlgo == SZ::ALGO_INTERP_LORENZO);
    ...
    if (sampling_num == conf.num) {
        conf.cmprAlgo = SZ::ALGO_INTERP;
        return SZ_compress_Interp<T, N>(conf, data, outSize, compr, offset);
    }
    ...
    if (useInterp) {
        ...
        conf = lorenzo_config;
        double tuning_time = timer.stop();
        return SZ_compress_LorenzoReg<T, N>(conf, data, outSize, compr, offset);
    }
